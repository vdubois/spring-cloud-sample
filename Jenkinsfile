node('master') {
  ...
  try {
    ...
    dir('spring-cloud-sample-config-server') {
      ...
      sh "docker run --rm -v /home/vdubois/work/jenkins/workspace/spring-cloud-sample/spring-cloud-sample-config-server:/tmp -w /tmp --volumes-from=${dataContainer} vdubois/maven:3.3.9-jdk8 mvn clean package docker:build"
    }
