...
def buildModule(moduleName) {
  dir(moduleName) {
    ...
    sh "docker run --rm -v ${env.WORKSPACE_DIRECTORY}/spring-cloud-sample/${moduleName}:/tmp -v /var/run/docker.sock:/var/run/docker.sock -w /tmp --volumes-from=SPRING-CLOUD-SAMPLE-MVN-DATA-CONTAINER vdubois/maven:3.3.9-jdk8 mvn clean package docker:build"
  }
}
