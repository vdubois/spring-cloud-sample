...
def buildModule(moduleName) {
  ...
  dir(moduleName) {
    ...
    stage 'BUILD_JAR'
      sh "docker run --rm -v ${jenkinsWorkspace}/spring-cloud-sample/${moduleName}:/tmp -v /var/run/docker.sock:/var/run/docker.sock -w /tmp --volumes-from=SPRING-CLOUD-SAMPLE-MVN-DATA-CONTAINER vdubois/maven:3.3.9-jdk8 mvn clean package docker:build -Dmaven.test.skip=true"
    stage 'BUILD_DOCKER_IMAGE'
      def pom = readMavenPom file: "${env.WORKSPACE_DIRECTORY}/spring-cloud-sample/${moduleName}/pom.xml"
