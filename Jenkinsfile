node('master') {
  ...
  try {
    ...
    dir('spring-cloud-sample-config-server') {
      ...
      sh "docker run --rm -v ${env.WORKSPACE_DIRECTORY}/spring-cloud-sample/spring-cloud-sample-config-server:/tmp -v /var/run/docker.sock:/var/run/docker.sock -w /tmp --volumes-from=${dataContainer} vdubois/maven:3.3.9-jdk8 mvn clean package docker:build"
    }
